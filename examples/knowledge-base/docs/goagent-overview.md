...
ares 是一个基于 Go 语言构建的通用 AI Agent 开发框架，提供 DAG 工作流编排、多 Agent 协作、记忆蒸馏、向量存储和事件溯源等核心能力。使用 PostgreSQL + pgvector 作为默认的持久化存储后端。
...
```bash
# 启动数据库
docker run -d --name ares-db -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=goagent -p 5433:5432 pgvector/pgvector:pg15
...
- 主仓库: https://github.com/anomalyco/ares
- 文档: docs/ 目录下的中英文文档
