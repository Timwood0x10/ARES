...
# Usage:
#   ./run.sh                    # start + tail logs
#   ./run.sh --stop             # stop running instance
#   ./run.sh --status           # check if running
#
set -e

CONFIG="${1:-./examples/quant-demo/config.yaml}"
DIR="$(cd "$(dirname "$0")" && pwd)"
LOG="$DIR/run.log"
PIDFILE="/tmp/quant-demo.pid"
...
if lsof -i ":$PORT" >/dev/null 2>&1; then
    warn "Port $PORT is in use. Use '$0 --stop' first."
fi

# Build.
info "Building mcp-null helper..."
go install ./cmd/mcp-null/ 2>/dev/null || true

info "Building quant demo..."
BUILD_OUT=$(go build -o /tmp/quant-demo ./examples/quant-demo/ 2>&1) || {
    fail "Build failed:"
    echo "$BUILD_OUT"
    exit 1
}
ok "Built to /tmp/quant-demo"

# Start.
info "Starting on port $PORT..."
/tmp/quant-demo -config "$CONFIG" -log "$LOG" > "$LOG" 2>&1 &
PID=$!
echo "$PID" > "$PIDFILE"
...
echo -e "    - Open Dashboard → Arena tab to see agent DAG"
echo -e "    - Click ☠Leader to assassinate, watch auto-recovery"
echo -e "    - Watch momentum/value factor rankings computed live"
echo ""
