...
6. 回归门 arena 接线 ✅（M4 三批；M-G2 起默认 AUTO-ARMED）：ArenaRegressionGate 进 lifecycle verify 管线（G1 guardrail → G2 shadow → G3 eval → arena regression → staging）——G3 eval 门打绝对分，本门打相对分：candidate vs active 的 PromptTemplate 在 eval_suite preserved cases 上 A/B（arena.RegressionTester 配对运行 + Welch t 检验），仅显著回退拒绝（Confident && NewAvg<OldAvg），平局放行交 staging。**默认 armed（eval_suite+LLM client 存在即开，regression_enabled 为 tri-state：nil=auto/false=显式退避+Warn/true=armed+缺件 fatal）**；每 check 2×runs LLM 轮。G3 同批 M-G1 拉齐：armed 但缺 infra → bootstrap fatal（不再 strict=false 静默放行）；built 门 runtime 恒 strict；eval_strict 只管"无 suite 是否 fatal"。
