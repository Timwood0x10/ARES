...
```bash
ares init myproj && cd myproj   # generates ares.yaml incl. a security.api_key
...
# bearer = security.api_key (dedicated HTTP credential, generated by ares init);
# when unset it falls back to llm.api_key. With NEITHER set (e.g. ollama,
# which needs no llm.api_key) the write gate answers 401 — deny-by-default,
# loopback included — so set security.api_key before serving.
...
# = the session answer on the L2 path; error carries the failure cause on
# FAILED)
curl -sS -H "Authorization: Bearer <security.api_key from ares.yaml>" \
