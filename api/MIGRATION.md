...
**The legacy client (`ares/api` package's `Client`, `Config`, `NewClient`) has been REMOVED.**

The following types/functions no longer exist and must be migrated to `ares/api/client`:
...
**Note:** The `ares/api` package still exists and provides the **Service layer** (`StartService`, `LoadServiceConfig`, `Service`, etc.) used by examples. Only the legacy *library-style client* was removed.

## 概述

本文档说明如何从旧的API（`ares/api`包中的老客户端）迁移到新的分层API（`ares/api/client`包）。
...
2. 新项目必须使用 `ares/api/client` 包
3. 仍在使用老类型的代码将无法编译

### 迁移检查清单

- [ ] 将 `import "ares/api"` （用于客户端）改为 `import "ares/api/client"`
...
A: **不可以。** 老客户端已删除，无法再同时使用。请完全迁移到新API `ares/api/client`。
