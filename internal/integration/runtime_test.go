...
func (a *integrationStatefulAgent) ReplayEvents(evts []*events.Event) error {
	a.replayedEvts = evts
	return nil
}
...
func TestRuntime_FullFailoverCycle(t *testing.T) {
	...
	// Step 2: Pre-populate events (simulating prior session).
	streamID := "leader-1"
	err := eventStore.Append(ctx, streamID, []*events.Event{
		{
...
func TestRuntime_MultipleAgentTypes(t *testing.T) {
	...
	// Pre-populate events for both agents.
	err := eventStore.Append(ctx, "leader-1", []*events.Event{
		{
			Type:    events.EventSessionCreated,
	...
	err = eventStore.Append(ctx, "sub-1", []*events.Event{
		{
			Type:    events.EventTaskCreated,
