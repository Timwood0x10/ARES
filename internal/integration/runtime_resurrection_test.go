...
func (a *resurrectionAgent) ReplayEvents(evts []*events.Event) error {
	a.mu.Lock()
	a.replayedEvts = evts
	a.mu.Unlock()
	return nil
...
// register agent -> start -> emit events -> kill -> verify resurrection -> verify state recovery.
func TestRuntimeResurrection_FullCycle(t *testing.T) {
	...
	// Pre-populate events to simulate prior session activity.
	err := eventStore.Append(ctx, "res-agent-1", []*events.Event{
		{
	...
	// Verify state was recovered from events: the restored agent should have
	// received the session_id from EventSessionCreated events.
	...
	assert.Equal(t, "session-001", restoredState["session_id"], "session_id should be recovered from events")
}
...
func TestRuntimeResurrection_MultipleAgents(t *testing.T) {
	...
	// Pre-populate events for all agents.
	for _, id := range agentIDs {
		err := eventStore.Append(ctx, id, []*events.Event{
			{
				Type:    events.EventSessionCreated,
...
func TestRuntimeResurrection_EventStoreUnavailable(t *testing.T) {
	...
	// The new agent should NOT have any restored state (no events to replay).
