...
// Returns the events that were appended for later verification.
func emitSessionEvents(
	...
	eventsList := make([]*events.Event, len(eventTypes))
...
// 1. Leader A emits session events to the event store.
// 2. Leader A "crashes" (we just stop emitting).
// 3. Leader B reads the event store and replays events.
...
func TestEventSourcing_FailoverRecovery(t *testing.T) {
	...
	// Leader A emits session lifecycle events.
	eventTypes := []events.EventType{
	...
	require.NoError(t, err, "leader A should emit events successfully")

	// Leader A "crashes". The events remain in the store.

	// Leader B starts and replays all events from the store.
	replayed, err := store.Read(ctx, streamID, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
	})
	require.NoError(t, err)
	require.Len(t, replayed, 5, "leader B should replay all 5 events")

	// Verify event types match the original sequence.
	assert.Equal(t, events.EventSessionCreated, replayed[0].Type)
	assert.Equal(t, events.EventMessageAdded, replayed[1].Type)
	assert.Equal(t, events.EventMessageAdded, replayed[2].Type)
	assert.Equal(t, events.EventTaskCreated, replayed[3].Type)
	assert.Equal(t, events.EventTaskCompleted, replayed[4].Type)
	...
	// Leader B can continue emitting events from where leader A left off.
	newEvents := []*events.Event{
		{Type: events.EventMessageAdded, Payload: map[string]any{"role": "user", "content": "follow-up"}},
	}
...
// TestEventSourcing_EventOrdering verifies that events are replayed in the
...
func TestEventSourcing_EventOrdering(t *testing.T) {
	...
	const eventCount = 10
	eventsList := make([]*events.Event, eventCount)
	...
	ascending, err := store.Read(ctx, streamID, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
	})
	require.NoError(t, err)
	...
	descending, err := store.Read(ctx, streamID, events.ReadOptions{
		FromVersion: 1,
	...
	// Read with limit.
	limited, err := store.Read(ctx, streamID, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
		Limit:       3,
	})
	require.NoError(t, err)
	assert.Len(t, limited, 3, "limit should cap the number of returned events")
...
func TestEventSourcing_ConcurrentAppend(t *testing.T) {
	...
	for g := 0; g < numGoroutines; g++ {
		...
		go func(idx int) {
			...
			for i := 0; i < eventsPerGoroutine; i++ {
				...
				err := store.Append(ctx, streamID, []*events.Event{event}, 0)
				if err == nil {
					successCounts[idx]++
				}
	...
	allEvents, err := store.Read(ctx, streamID, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
	})
	require.NoError(t, err)
	...
	assert.Len(t, allEvents, totalExpected, "store should contain all appended events")
...
// TestEventSourcing_SubscribeAndReplay verifies that subscribers receive events
...
func TestEventSourcing_SubscribeAndReplay(t *testing.T) {
	...
	// Subscribe to events on this stream before emitting.
	subCh, err := store.Subscribe(ctx, events.EventFilter{
	...
	// Emit events one at a time so the subscriber can consume each before
	...
	emittedEvents := []emittedEvent{
		{events.EventSessionCreated, map[string]any{"session_id": streamID}},
	...
	for i, ee := range emittedEvents {
		appendErr := store.Append(ctx, streamID, []*events.Event{
...
func TestEventSourcing_FailoverWithPartialProgress(t *testing.T) {
	...
	// Leader A emits the first 3 events of a planned 6-event sequence.
...
func TestEventSourcing_FailoverMultipleStreams(t *testing.T) {
	...
	// Leader A emits events to two different streams.
	err := store.Append(ctx, streamA, []*events.Event{
		{Type: events.EventSessionCreated, Payload: map[string]any{"stream": "A"}},
		{Type: events.EventMessageAdded, Payload: map[string]any{"content": "A1"}},
	}, 0)
	require.NoError(t, err)

	err = store.Append(ctx, streamB, []*events.Event{
		{Type: events.EventSessionCreated, Payload: map[string]any{"stream": "B"}},
		{Type: events.EventMessageAdded, Payload: map[string]any{"content": "B1"}},
		{Type: events.EventMessageAdded, Payload: map[string]any{"content": "B2"}},
	}, 0)
	require.NoError(t, err)
	...
	eventsA, err := store.Read(ctx, streamA, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
	})
	require.NoError(t, err)
	require.Len(t, eventsA, 2, "stream A should have 2 events")
	assert.Equal(t, "A1", eventsA[1].Payload["content"])

	eventsB, err := store.Read(ctx, streamB, events.ReadOptions{
		FromVersion: 1,
		Direction:   events.ReadAscending,
	})
	require.NoError(t, err)
	require.Len(t, eventsB, 3, "stream B should have 3 events")
	...
	// ReadAll should return events from both streams.
	allEvents, err := store.ReadAll(ctx, events.ReadOptions{
		Direction: events.ReadAscending,
	})
	require.NoError(t, err)
	assert.Len(t, allEvents, 5, "ReadAll should return events from both streams")
}
