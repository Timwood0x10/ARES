...
func (f *fakeEvaluator) Evaluate(_ context.Context, _ ares_eval.TestCase, _ ares_eval.TestResult) ([]ares_eval.EvalScore, error) {
	if f.failWith != nil {
		return nil, f.failWith
	}
	return []ares_eval.EvalScore{{Metric: "accuracy", Score: f.score}}, nil
}
...
func newTestEvalGate(t *testing.T, evaluatorScore float64, evaluatorErr error) *EvalGate {
	...
	suite := ares_eval.TestSuite{TestCases: []ares_eval.TestCase{
...
func TestAverageScores(t *testing.T) {
	t.Run("empty inputs return zero", func(t *testing.T) {
		...
		assert.Equal(t, 0.0, averageScores([][]ares_eval.EvalScore{{}}, 0))
	})
	t.Run("mean across cases and metrics", func(t *testing.T) {
		scores := [][]ares_eval.EvalScore{
	...
	t.Run("scores present but no metrics counted", func(t *testing.T) {
		assert.Equal(t, 0.0, averageScores([][]ares_eval.EvalScore{{}, {}}, 2))
	})
}
...
func TestEvalGate_PassThroughWithoutInfrastructure(t *testing.T) {
	...
	t.Run("empty suite", func(t *testing.T) {
		runner, err := ares_eval.NewAgentTestRunner(&fakeExecutor{output: "ok"})
		require.NoError(t, err)
		g := NewEvalGate(ares_eval.NewEvaluatorRegistry(), runner, ares_eval.TestSuite{}, DefaultEvalGateConfig())
...
func TestEvalGate_ScoringPath(t *testing.T) {
	...
	t.Run("multi-evaluator path averages across evaluators", func(t *testing.T) {
		...
		runner.SetRegistry(registry)
		suite := ares_eval.TestSuite{TestCases: []ares_eval.TestCase{{ID: "case-1", Input: "hello"}}}
