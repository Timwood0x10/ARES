...
func buildMutator(cfg SystemConfig) (*mutatorResult, error) {
	...
	if cfg.EnableExperienceGuidedMutation && cfg.GuidanceProvider != nil {
		el.Info(nil, "buildMutator", "experience-guided mutation requested; provider wired",
		...
		el.Warn(nil, "buildMutator", "experience-guided mutation requested but no GuidanceProvider set")
	}
...
func buildAdapterOptions(cfg SystemConfig) ([]GenomeAdapterOption, *scoring.TieredScorer, *scoring.Budget, *scoring.ScoreCache, error) {
	...
	if cfg.MemoryAwareScoringConfig.Enabled && cfg.MemoryExperienceProvider != nil {
		...
		if err != nil {
			el.Warn(nil, "buildAdapterOptions", "failed to create memory-aware scorer, skipping",
				"error", err)
		} else {
...
func buildShadowEvaluator(cfg SystemConfig, baseStrategy *mutation.Strategy) *ShadowEvaluator {
	...
	el.Info(nil, "buildShadowEvaluator", "shadow evaluation enabled",
...
func wrapGuidanceProvider(provider GuidanceProvider, raw *mutation.Mutator) genome.MutatorInterface {
	...
	if err != nil {
		el.Warn(nil, "wrapGuidanceProvider", "failed to create ExperienceGuidedMutator, falling back to raw mutator",
			"error", err)
		return raw
	}
	el.Info(nil, "wrapGuidanceProvider", "experience-guided mutation enabled",
