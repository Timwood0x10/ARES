...
func TestEnterprise_GenerateReport(t *testing.T) {
	...
	reportBuf.WriteString(fmt.Sprintf("- LLM API: %s\n", llmCfg.BaseURL))
	...
	reportBuf.WriteString("| 指标 | 纯内存版 | 企业版 |\n")
	reportBuf.WriteString("|------|---------|-------|\n")
	reportBuf.WriteString("| Embedding | qwen3-embedding:0.6b (1024维) | qwen3-embedding:0.6b (1024维) |\n")
	reportBuf.WriteString("| Token压缩率 | 95%+ | 需sensenova实测 |\n")
	reportBuf.WriteString("| 上下文控制 | 恒定 ~300 tokens | 需sensenova实测 |\n")
	reportBuf.WriteString("| 主题保留 | ~30% (规则提取) | 需sensenova实测 |\n")
	reportBuf.WriteString("| 详细日志 | report_real_embedding.md | enterprise_benchmark_log.txt |\n\n")
