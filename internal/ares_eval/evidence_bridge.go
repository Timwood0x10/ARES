...
// dimensions) into a structured eval.Evidence diagnosis (Ch.8: verification
...
func (b *DimensionJudgeBridge) ToEvidence(taskID, role string, resp *DimensionJudgeResponse) (*eval.Evidence, error) {
	if resp == nil {
	...
	case isUncertainReason(resp.Reason):
		ev.Verdict = eval.VerdictUncertain
	case ev.HasFailure():
		ev.Verdict = eval.VerdictFail
	default:
		ev.Verdict = eval.VerdictPass
	}
	return ev, nil
}
...
func llmJudgeItem(name string, score float64, max int) []eval.EvidenceItem {
	status := "passed"
	if score < float64(max)*2/3 {
		status = "failed"
	}
	return []eval.EvidenceItem{{
