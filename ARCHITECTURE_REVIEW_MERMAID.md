...
2. **introspect 无鉴权**：HTTP API 仅注释声明"trusted operators only"，可读全部 agent/任务/事件/决策数据。建议默认 bind 127.0.0.1 + 简单 token。
   - ✅ M-S1 已整改（2026-09-10）：默认 bind 已是显式 `127.0.0.1`（原为 "localhost"，现改为字面 loopback IP，杜绝 hosts 重映射）；新增 `introspect.token`（非空时非本机读请求必须携带 Bearer token，常量时间比较；本机保持开放），启动日志打印暴露面状态（bind/token/auth），token 空时启动 Warn。HTTP 测试锁定（routes_registry_test.go: TestIntrospectTokenGatesNonLoopbackReads 等）。
   - ✅ M-S3 已核对（2026-09-10）：**syscall 与 HTTP 写路径不共用同一授权源，且这是设计事实而非不一致**——两者守护不同的主客体：HTTP 面（kill/resume/chaos/evolution approve/tasks/graphs/tools call）统一走 checkAuth（JWT PermWrite ← security.jwt_secret+auth_enabled，或 ARES_API_KEY，deny-by-default，共享 AuditLogger），认证的是外部 operator；agentsyscall.Kernel（spawn_agent/create_task/create_plan/ask_agent）是进程内 agent 经 sub.ToolBinder 的调用，其"授权"是 Kernel 强制的 capability（L2-routable 校验）+ 配额（agentfabric resource budget，ErrResourceQuotaExceeded）+ 溯源（kctx.CallerID 压过 LLM 参数，无法伪造 parentage），两侧各自 fail-closed。已核实无 HTTP 旁路直达 syscall：/api/tools/call 只服务 api_tools.Registry（builtins+MCP），syscalls 只绑定在 agent 的 LLM ToolBinder 上；HTTP 建任务的唯一入口 POST /api/tasks、/api/graphs 均为 authWrite。**结论：无需"拉齐"——给进程内 syscall 套 HTTP 身份层是类别错误；现状两侧授权源各自完整。**
   - ✅ M-S3 已核对（2026-09-10，chaos 鉴权级别）：/api/chaos/* = **AuthWrite + handler 内 PermAdmin RBAC**（random-kill/kill-all/recover 需 admin 角色；stop 额外要求 X-Chaos-Token，空 token 时端点 503 不可用）。非 AuthLocal：远程 admin 运维是设计意图，风险由 admin 角色门槛 + stop token + 审计三重控制；注册表如实标注 authWrite（routes_registry_test.go: TestRegistryChaosRequiresCredentials 锁定 deny-by-default）。

### P2（结构性债务，收敛期内处理）
3. **入口层巨型文件**：serve.go runServe 400+ 行、agent.go 2804 行手写路由 switch，新增端点易漏鉴权分支（本项目已有 JWT 中间件，宜上 mux 分层）。
   - ✅ M-S2 已整改（2026-09-10，分发骨架部分）：agent.go 手写路由 switch 已替换为端点注册表 `actionRoutes`（[]routeSpec{Method, Path, Auth: none|read|write|local, Handler}）+ 统一 ServeHTTP 分发（按注册表匹配 → 按声明级别过鉴权 → 调 handler）。18 条路由对照原 switch 分支零丢失（TestActionRoutesRegistry golden diff），handler 函数本体一行未动、未拆文件；每端点鉴权级别显式声明（chaos/tasks/graphs/mcp-call=write，tools 清单/introspect feed/cost/control-server 尾=read，panel/metrics/根跳转/非 API 尾=none）。启动日志打印注册表 auth 级别分布。serve.go 的 HTTP 面复用同一 actionHandler（无第二套路由）；introspect.ControlServer 内部虽还有一只手写 switch（control.go:173），但其全部路由均为只读 GET（唯一的 POST 分支刻意 405）且整体位于 actionHandler 注册表 `/api/...` authRead 门之后——它不做任何鉴权决策，重构它不会改变安全语义，故本批只登记不重构（如未来直接暴露 ControlServer 需先补注册表）。文件拆分按计划留待 M-C2。
