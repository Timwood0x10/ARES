...
> 状态：**已实施（2026-08-15）**——第一批：SkillCatalog 核心（SourceManager/Indexer/Discovery/Loader/ToolResolver/Experience）+ Catalog 门面 + 生产接线（serve.go wireSkillCatalog → memoryManager 常驻技能块）。第二批：`~/.ares/config.toml` `[[skill_sources]]` 解析、MCP 懒连接（`Catalog.Activate` → `ConnectServer`）、Experience JSON 持久化、envcap 聚合桥接、hash 变更检测（`DetectIndexChanges` + `Refresh`）。第三批：Git 源（`SyncGitSource`）、HTTP/OCI 源（`FetchHTTPManifest`）、SQLite FTS5 全文检索（`FTS5Index` + Discovery 回退）、MCP listChanged 增量重索引（`MCPManager.SetToolChangeHandler` → `Catalog.Refresh`）。第四批（code_review 修复，本日）：`Refresh` 与 `Build` 语义对齐（重拉 http 源 + 重建 FTS5 关旧索引 + 重 seed registry）、Catalog 全量 RWMutex 并发保护（写锁 swap / 读锁 Search·Load·All·ResolveTools）、git sync 2 分钟超时防阻塞启动、并发 Refresh 与 Build→Refresh 一致性测试（-race）。第五批（不空转接线，本日）：`skill_search`/`skill_load`/`skill_activate`/`skill_list`/`skill_experience` 五个 `core.Tool` 注册进 serve 共享 registry 并 re-bridge toolBinder（LLM 在 agent 主循环中可实际 Discover→Load→Activate，闭合 §10 主循环；MCP 懒连接由 `skill_activate` 触发）；`skills.Registry.SetDetailLoader` + `SeedRegistry` 注入让 `LoadDetail` 按需返回 SKILL.md body（Level-1 渐进披露不再断链）。第六批（Experience 记录侧闭环，本日）：复用现有机制而非新造钩子——`cmd/ares` 给 planner 注入 `leader.WithExperienceLocator`（现有注入点，nil 时行为不变），任务创建时把 BestMatch 命中的 skill ID 预填进 `task.UsedExperienceID`；新建 `SkillOutcomeRecorder` 订阅现有 `EventSubTaskResult` 事件流（sub agent 完成任务时已携带 task+success），把 `{skill, task_pattern, success}` 写入 catalog 的 Experience store，完成 §11 记录侧闭环。低风险保证：零侵入 leader/sub/planner 代码、失败仅日志、nil catalog/store 离线 no-op、并发消费有 -race 守护。41 组测试守护，`make fmt && make check` 全绿（166 包）。
...
**task_pattern 精确化（2026-08-15）**：planner 创建 task 时把原始任务输入存入 `task.Payload["task_desc"]`；`SkillOutcomeRecorder.skillTaskPattern` 以精确描述优先（trim 后截断至 256 字符，防止完整用户输入明文落盘 `experience.json`），无则回退 `AgentType` → `subAgentID` → `"default"`。`BestMatch` 匹配语义升级为关键词重叠评分：短 pattern（<4 token，如回退的 `agent_top`）保持双向子串语义；长 pattern（完整任务描述）按 token 重叠率评分且需 ≥0.5 阈值才算命中，避免两个长描述仅共享偶发词汇时的虚假匹配（如 `skill_experience` 工具与 locator 用完整用户输入查询的场景）。**已实现并测试守护（`patternMatchScore` experience.go:245-268 + `TestBestMatchKeywordScoring`：长 pattern 重叠命中 / 单词汇虚假拒绝 / 短 pattern 子串保持）。**
