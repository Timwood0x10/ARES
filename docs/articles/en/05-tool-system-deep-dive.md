# ares Architecture Deep Dive (V): Tool Invocation Layer -- Four Paths and One Safety Net (0.3.x)
...
>
> 0.3.x update: The tool system is deeply integrated with the Capability Fabric. The ToolExpander interface lets runtime-discovered skill names be resolved into LLM tool definitions on the fly. The five-piece catalog toolset (skill_search/load/activate/list/experience) implements progressive disclosure — even 1000 tools don't enter context. The Kernel Scheduler's capability-aware scoring directly depends on tool-registered capability declarations.
