# GoAgentX Architecture Deep Dive (V): Tool Invocation Layer -- Three Paths and One Gap
...
MCP (Model Context Protocol) is Anthropic's standardized tool protocol using JSON-RPC over stdin/SSE. GoAgentX's MCP adapter wraps external tools as standard `core.Tool`:
...
func NewMCPTool(client *MCPClient, serverName string, def *MCPToolDef) *MCPTool {
    ...
    // needs conversion to GoAgentX's ParameterSchema
