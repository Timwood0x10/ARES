...
最让我满意的不是功能数量，是并发模型和热重载。errgroup + WaitGroup + semaphore + stepDone 通道这一套组合拳，配合双模式文件监控和原子替换，是我花了最多心思调出来的。虽然中间踩过 C6、C7、H2-H4、M5-M9 这些坑——但每次修完都更确定一件事：**Go 的并发，设计之初就要想清楚，不能靠后期 Debug 打补丁。**
