...
很多人第一次看到 GoAgentX 的存储设计时，第一反应是"不就向量检索吗，用一个纯向量数据库不就行了？"——这个反应我太熟悉了，因为我一开始也是这么想的。
...
在 1024 维（GoAgentX 使用的维度）空间中，任意两条无关联的向量之间的余弦相似度也往往在 0.7-0.8 之间。这意味着当你往一个向量桶里塞了十万条数据，搜"Python 安装依赖报错"时，返回的结果里可能前十名中有五条是：
...
GoAgentX 的选择：**把向量存在 pgvector 插件，结构化元数据存在同一 PostgreSQL 的关联表。一次检索在 SQL 层面完成向量 + 元数据的联合过滤和排序。**
