...
**PlannerGenome**（`internal/runtime/evolution/genome/genome.go`）调优规划参数：`Strategy ("balanced"/"architecture-first"/"memory-first")`、`MaxSources [3,30]`、`MinRelevance [0.1,0.9]`。启发式适应度对极端策略施加惩罚。
