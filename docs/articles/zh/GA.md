...
基因（一个个体 / 一条策略）定义为 `mutation.Strategy`（[types.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/mutation/types.go)），核心字段：
...
`NewPopulation`（[population.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/population.go#L101-L151)）：
...
`doEvolve`（[population.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/population.go#L258-L428)）是核心循环，流程为：
...
选择器实现 `Selection` 接口（`Select(ctx, population, n)`），由 `buildSelector` 按 `SelectionStrategy` 分发（[selection.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/selection.go)）：
...
交叉在 `genome.Crossover`（[crossover.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/crossover.go)），参数重组方式由 `WithCrossoverType` 配置：
...
`Mutator.Mutate`（[mutator.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/mutation/mutator.go#L86-L196)）按概率为每个子代选择变异类型：
...
池为空时该类型概率被重分配。`AdaptiveDistribution`（[adaptive_distribution.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/mutation/adaptive_distribution.go)）可基于历史胜负动态调整这三类概率（参数 0.30–0.90、提示词 0.05–0.50、工具 0.05–0.50，含探索下限 0.03、学习率 0.10）。
...
集中在 [adaptive.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/adaptive.go) 与 [population_guard.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/population_guard.go)。
...
- 哨兵值 `ScoreUnevaluated = -1.0`，`IsScoreEvaluated(score) = score >= 0`（[score.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome/score.go)）。
...
[DreamCycle](file:///Users/scc/go/src/goagent/internal/ares_evolution/dream_cycle.go) 是进化中枢，`EvolutionMode` 二选一：
...
`Run()` 的主流程（[dream_cycle.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/dream_cycle.go#L284-L343)）：
...
GA 路径 `runGAEvolution`（[dream_cycle_ga.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/dream_cycle_ga.go#L40-L117)）：
...
`deployWinner`（[dream_cycle.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/dream_cycle.go#L486-L606)）统一处理 GA/ES 的部署后逻辑：守卫后置检查 → 影子评估（ShadowEvaluator）→ 尺寸校验 → `ActiveStrategyManager.Deploy` → 指标埋点 → 记录 hint outcome。
...
`EvolutionScheduler.Register()`（[scheduler.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/scheduler.go#L297-L320)）订阅 `ares_callbacks.EventAgentEnd`。`OnAgentEnd` 在任务完成时触发，异步经 errgroup 运行 Adapter（`adapter.Run`）。

`shouldEvolve` 判定（[scheduler.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/scheduler.go#L336-L399)）：
- 最小间隔保护（默认 5 分钟）。
...
`GenomePopulationAdapter`（[genome_wiring.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome_wiring.go)）实现 `AdapterRunner`，把 `genome.Population` 接入调度器。其 `Run`（[genome_wiring_run.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/genome_wiring_run.go#L45-L102)）：
```
...
### 11.7 守卫（Guardrails）
`EvolutionGuardrails`（[guardrails.go](file:///Users/scc/go/src/goagent/internal/ares_evolution/guardrails.go)）：
...
[api/evolution](file:///Users/scc/go/src/goagent/api/evolution/evolution.go) 封装内部实现，暴露：
