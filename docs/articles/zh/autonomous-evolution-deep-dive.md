...
### 9.8 EvolutionStore：DB 持久化——告别 Placeholder

好了，说了这么久的"最后一公里"，终于接上了。

上一节提到的 `getCurrentStrategy()` placeholder 问题，根源在于**没有一个真实的策略存储后端**。现在 `internal/evolution/evolution_store.go` 彻底解决了这个问题。

#### 接口定义

先看 `interfaces.go` 中新增的 `StrategyStore` 接口：

```go
// interfaces.go — StrategyStore 接口
type StrategyStore interface {
    // GetActive 返回当前部署的策略。如果没有存储过则返回 nil。
    GetActive(ctx context.Context) (*Strategy, error)

    // SetActive 将一个策略持久化为当前活跃部署。
    SetActive(ctx context.Context, s Strategy) error

    // List 返回按版本降序排列的最新 N 个策略。
    List(ctx context.Context, n int) ([]Strategy, error)
}
```

三个方法，职责清晰：读当前活跃策略、写新活跃策略、查历史版本列表。这就是完整的 CRUD 子集——够用且不多余。

#### EvolutionStore 实现

`EvolutionStore` 是基于关系型数据库的实现：

```go
// evolution_store.go — EvolutionStore 核心结构体
type EvolutionStore struct {
    db StoreDB
}
```

注意它依赖的是 `StoreDB` 接口而不是具体的 `*sql.DB`：

```go
// evolution_store.go — StoreDB 抽象
...
`*sql.DB` 和 `*sql.Tx` 都天然满足这个接口。这意味着你既可以用普通连接也可以用事务——在需要原子性地更新多个策略时，事务模式就派上用场了。

#### GetActive：读取当前部署策略

```go
// evolution_store.go — GetActive
...
`is_active = TRUE` + `ORDER BY version DESC LIMIT 1`——这个查询的设计意图很明确：永远只返回**最新版本的活跃策略**。即使因为某种原因有多条 `is_active=TRUE` 的记录（数据异常），也只会取版本号最高的那个。

Params 字段用 JSON 存储（`json.Marshal` / `json.Unmarshal`），这是处理 schema-free 参数的最灵活方案。不同策略可能有完全不同的参数集合（有的有 `temperature`，有的有 `max_steps`，还有的自定义字段），JSON 列天然适配这种多态性。

#### SetActive：Upsert 语义

这是最关键的方法——它实现了 **Upsert（INSERT OR UPDATE）** 语义：

```go
// evolution_store.go — SetActive
func (s *EvolutionStore) SetActive(ctx context.Context, st Strategy) error {
    ...
    s.db.ExecContext(ctx,
        `UPDATE evolution_strategies SET is_active = FALSE WHERE is_active = TRUE`)

    // 第二步：插入新策略（如果 ID 已存在则 UPDATE）
    query := `
        INSERT INTO evolution_strategies (...) VALUES (...)
...
两步操作的顺序很重要：先 deactivate 全部，再 activate 新的。这保证了任意时刻**最多只有一个 `is_active=TRUE`** 的记录。`ON CONFLICT DO UPDATE` 处理了"同一策略被多次 SetActive"的场景——比如进化出了更好的 score，需要更新已有记录。

#### 对接 DreamCycle

有了 EvolutionStore，之前的 placeholder 终于可以退休了：
...
### 9.9 序列化与快照系统：断点续跑与状态审计

进化跑了 1000 代之后，你想看看中间某一代的种群长什么样？或者进程被 kill 了，想从第 500 代继续？又或者审计部门问"上周三凌晨三点你们的 Agent 自己改了什么配置"？——这些需求都需要**完整的序列化能力**。

`internal/evolution/serialize.go` 就是干这件事的。

#### EvolutionRunSnapshot：完整状态捕获

```go
// serialize.go — EvolutionRunSnapshot
type EvolutionRunSnapshot struct {
    Config     SystemConfig         `json:"config"`      // 系统配置
    Generation int                  `json:"generation"`  // 当前代数
    Agents     []*mutation.Strategy `json:"agents"`      // 所有个体（深拷贝）
    Lineages   []StrategyLineage    `json:"lineages,omitempty"` // 谱系记录
}
```

四个字段，覆盖了重建一次完整进化运行所需的全部信息。注意 `Agents` 是深拷贝——不是指针引用。这意味着你加载 snapshot 之后修改 Agent，不会影响原始文件里的数据。

#### SaveEvolutionRun：一键持久化

```go
// serialize.go — SaveEvolutionRun
func SaveEvolutionRun(filepath string, system *WiredEvolutionSystem) error {
    if system == nil { return fmt.Errorf("system must not be nil") }
    if system.Population == nil { return fmt.Errorf("system population must not be nil") }

    // ★ 核心：调用 Population.Snapshot() 获取线程安全的深拷贝
    ...
    if err != nil { return fmt.Errorf("marshal evolution snapshot: %w", err) }

    return os.WriteFile(filepath, data, 0644)
}
```

`json.MarshalIndent` 输出带缩进的 JSON——不是出于美观考虑，而是为了** git diff 友好**。如果你把 snapshot 文件纳入版本控制（虽然通常不应该），缩进格式让 diff 更易读。

#### LoadEvolutionRun：从 JSON 恢复

```go
// serialize.go — LoadEvolutionRun
func LoadEvolutionRun(filepath string) (*EvolutionRunSnapshot, error) {
    data, err := os.ReadFile(filepath)
    if err != nil { return nil, fmt.Errorf("read evolution snapshot: %w", err) }
    ...
    // 防御性初始化：确保 slice 不为 nil
    if snapshot.Agents == nil { snapshot.Agents = []*mutation.Strategy{} }
    if snapshot.Lineages == nil { snapshot.Lineages = []StrategyLineage{} }
...
go func() {
    ...
    for range ticker.C {
        path := fmt.Sprintf("evolution_snapshot_gen%d.json", gen)
        if err := serialize.SaveEvolutionRun(path, system); err != nil {
...
// 场景 2：断点续跑
snapshot, _ := serialize.LoadEvolutionRun("evolution_snapshot_gen500.json")
// 从 snapshot.Agents 和 snapshot.Config 重建 Population...
// 然后从 snapshot.Generation + 1 继续进化

// 场景 3：审计查询——查看某次运行的完整进化历史
snapshot, _ := serialize.LoadEvolutionRun("audit_2024-01-15.json")
for _, lineage := range snapshot.Lineages {
    fmt.Printf("%s → %s (%s)\n", lineage.ParentID, lineage.ChildID, lineage.MutationType)
}
```

#### 完整的数据流（含持久化）

加上 DB 持久化和序列化之后，完整的数据流变成这样：
...
graph TD
    ...
    subgraph "💾 持久化层"
        ...
        BEST --> STORE["StrategyStore.SetActive()<br/>DB 持久化"]
        NEXT --> |"SaveEvolutionRun()"| FILE["JSON 快照文件<br/>断点续跑 / 审计"]
    end

    subgraph "🔄 恢复层"
        STORE -.-> |"GetActive()"| RESTORE1["重启后恢复活跃策略"]
        FILE -.-> |"LoadEvolutionRun()"| RESTORE2["断点续跑<br/>从任意代恢复"]
    end

    style STORE fill:#fff9c4
    style FILE fill:#e1f5fe
    style RESTORE1 fill:#c8e6c9
    style RESTORE2 fill:#c8e6c9
```

三条持久化路径各有分工：
- **Genealogy Recorder**：内存中维护实时谱系（轻量、低延迟）
- **StrategyStore (DB)**：持久化当前最佳策略（跨重启存活、支持多实例共享）
- **JSON Snapshot File**：完整状态归档（断点续跑、历史审计）
...
而这次的大规模改进让我学到了另一件事：**一个好的架构设计要为"还没发生的需求"留好接口。** `StrategyStore` 接口在最初设计时只是一个占位符，但因为接口定义清晰（GetActive/SetActive/List 三个方法），当真正需要 DB 持久化时，实现层可以独立开发而不影响任何上层代码。DreamCycle 不关心策略从哪来——它只调用 `getCurrentStrategy()`，而那个方法的内部实现从硬编码切换到 DB 查询，对调用方是完全透明的。

同样地，`Population.Snapshot()` 方法的加入看似只是加了一个"读操作"，但它解锁了两个完全独立的高级功能：序列化快照（serialize 包）和自动谱系记录（RecordPopulationLineage）。一个方法，两种用途——这就是好的抽象的力量。
...
你现在回头看这套进化系统——Callback、FeedbackService、Arena、DreamCycle、genome 包（4 个文件 2000+ 行）、genome\_wiring（564 行）、再加上 mutation 包本身……加起来多少行？光 `internal/evolution/` 底下就十几个文件了。你可能在想：
...
GoAgentX 的自主进化系统不是什么黑科技。它就是把生物进化论的最核心思想——**变异、选择、遗传、交叉、持久化**——翻译成了代码：
