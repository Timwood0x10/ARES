...
| 维度 | Workflow Engine | Workflow Engine v2 (MutableDAG) | Graph |
|------|----------------|-------------------------------|-------|
| **定义方式** | YAML/JSON 配置 | YAML/JSON + 运行时变更 | Go 代码 |
| **执行单元** | Step (Agent) | Step (Agent) | Node (Tool/Agent) |
| **流程类型** | 静态，预定义 | 运行时可变 | 动态，运行时决策 |
| **适用场景** | 预定义工作流 | 运行中需要演进的工作流 | Tool 组合、条件分支 |
| **调度策略** | 依赖驱动 + FIFO | 依赖驱动 + FIFO + 变更重算 | 可插拔调度器 |
| **条件分支** | 不支持 | 不支持 | 支持（`Edge.Condition`） |
...
1. **条件分支丢失**：转换为 Workflow 会丢失 `Edge.Condition` 信息 -- Graph 原生支持边上的 `IfFunc` 条件，Engine 完全没有此能力。
2. **调度策略不同**：Graph 需要支持可插拔的运行时调度决策（优先级、短任务优先），不仅仅是依赖驱动排序。
3. **执行模型不同**：Graph 使用单线程 BFS + `readySet`；Engine 使用 `errgroup` + semaphore 并行。

> **注意**：Workflow Engine v2（MutableDAG + DynamicExecutor）现已支持运行时图变更，对于只需要动态增删节点/边而不需要条件分支的场景，差距已缩小。
