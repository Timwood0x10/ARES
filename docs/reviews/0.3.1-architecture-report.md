...
> **结论**：✅ **可以发布 0.3.1**（原"冻结清单需更新"项已修复；CHANGELOG 收敛条目与活文档漂移已于 2026-09-12 补齐，见第12节更新）
...
> **后续复核（2026-09-13，来自 `0.3.1-final-deep-review.md`）**：上表"全绿"结论需加限定——
> `internal/kernel` 的 `TestL2Graph_BurstGrowthConvergesThroughEvents` 在**全仓并发负载**下是
> flaky（隔离跑不复现），根因是 `planprojection` 尾部丢弃补偿不闭环（该 review 记为 **F-21**，
> P0）。**该根因已修复**（补偿定时器改为自重装 + `checkDrops` 增加丢弃版本比较）。
> 包数现已增长为 `./...` = 137、`./internal/...` = 131。
...
- `storage/ExecWithTenant` / `QueryWithTenant` 零生产调用 — #36 Phase 2/3 descope
  - **后果补充（2026-09-13，同日第三批处置后更新）**：零调用意味着 RLS 策略纯装饰——应用以表
    owner 连接（无 `FORCE ROW LEVEL SECURITY` 时 owner 完全绕过策略），且从不绑定 `app.tenant_id`
    GUC。租户隔离因此 100% 依赖每条 SQL 手写 `tenant_id` 谓词，**漏写即无第二道防线**。已核实的
    具体漏写：`conversation_repository.GetByID` 曾无租户谓词（跨租户读，已修）。处置结果：
    (a) `user_profiles`/`sessions`/`recommendations`/`events`/`agent_checkpoints`/
    `evolution_strategies` 六表已全部补 `tenant_id NOT NULL DEFAULT 'default'` 列
    （`ADD COLUMN IF NOT EXISTS` 幂等 backfill；`events`/`agent_checkpoints` 仅为 schema
    就绪，读写路径尚不过滤——见 migrate.go 注释）；(b) 六条装饰性 RLS 策略按签字方案 B
    （`plan/0.3.1plan/tenant_isolation.md` 残骸清除）改写为 `DISABLE ROW LEVEL SECURITY` +
    `DROP POLICY IF EXISTS`（幂等、清除存量库残留），回归由 `migrate_rls_guard_test.go`
    静态守卫锁定；未来启用 RLS 的前置条件清单保留在 plan 文档。
