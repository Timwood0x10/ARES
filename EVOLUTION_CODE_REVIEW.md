...
### 🟡 P2 — fallback 单 patch 丢弃 rollback（`patch.go:229`）✅ 已修复
...
**已修复（第 14 轮）**：fallback 分支改为与主路径一致——失败时若 executor 返回 rollback，用 `r.fallback.Apply(ctx, *rollback)` 自身应用它（rollback 目标为 fallback-only 键，无精确 executor），再返回原错误。并补 `TestRegistry_Apply_FallbackFailureWithRollback` 锁定该行为。
...
### 🟡 P2 — `PlannerGenome` 是冗余死代码 ✅ 已修复（删除）
`provide_new_evolution.go:120-128` 注册 `PlannerGenome` 但无对应 differ（`diffReg` 未注册 `NewPlannerDiffer`），`generateDiffPatches` 对其 `continue`（`genome_wiring_system.go` 按 name 查 differ，查不到跳过）。planner 维度已由 `KnowledgeDiffer`（`knowledge.planner.*`）覆盖。
**已修复（第 14 轮）**：删除冗余类型 `internal/evolution/genome/planner_genome.go`（全仓无生产/测试/example 引用，git 可恢复），并移除 `provide_new_evolution.go` 中的注册块与误导性注释。维度已由 knowledge 覆盖，删除无行为影响。
