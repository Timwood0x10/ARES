// serve_chaos_domain — the chaos subsystem wiring for serve: shadow sandbox
// loop, live chaos loop with its guard, and the emergency-stop control,
// split out of serve.go (M-C2; this was serve_chaos.go before the merge).
// Function bodies are moved verbatim; only the file boundary is new.
package main
