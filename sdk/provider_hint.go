...
func providerKeyHint(provider core.LLMProvider, apiKey string) string {
	if apiKey != "" {
		return ""
	}
	envVar := map[core.LLMProvider]string{
		core.LLMProviderOpenAI:     "OPENAI_API_KEY",
		core.LLMProviderAnthropic:  "ANTHROPIC_API_KEY",
		core.LLMProviderOpenRouter: "OPENROUTER_API_KEY",
	}[provider]
	if envVar == "" {
